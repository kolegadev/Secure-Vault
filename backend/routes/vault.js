  ...
  try {
    ...
    if (!(provider instanceof LuksProvider)) {
      return res.status(400).json({
        success: false,
        error: { code: 'NOT_SUPPORTED', message: 'Volume creation is not supported by the current vault provider' },
      });
    }
    ...
    logger.info({}, 'Vault volume created');
...
router.post('/keyslot', requireAuth, async (req, res, next) => {
  try {
    ...
    if (!(provider instanceof LuksProvider)) {
      return res.status(400).json({
        success: false,
        error: { code: 'NOT_SUPPORTED', message: 'Keyslot management is not supported by the current vault provider' },
      });
    }
